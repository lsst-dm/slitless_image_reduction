...
if __name__ == "__main__":
    ...
    for item, rep  in enumerate(reps):
        ...
        for reference in targets:
            ...
            pixcrd = np.array([[found.field('X_IMAGE')[0], found.field('Y_IMAGE')[0]]]
                              , np.float_)
            world  = w.wcs_pix2world(pixcrd, 1)
            ra     = world[0][0] 
            dec    = world[0][1]
            name   = reference[0]
            data.append(list([name,date,exptime,band,ra,dec,airmass,outhum,outpress,wndspeed]) + list(found[0]))
            if item==0 :
                columns = ('name','date', 'exptime', 'band','ra','dec','airmass','outhum','outpress', 'wndspeed') + found.dtype.names
