...
class telinst(object):
        ...
        for amp in self.amp:
            ...
            if((x_coord >= x_start) & (x_coord < x_end)
               & (y_coord >= y_start) & (y_coord < y_end)):
                logging.debug(str('Coordinate (x,y) : '+str(y_coord) +
                                  ' '+str(x_coord)+' '+' is in amp '+str(amp)))
                return amp
    ...
    def IlluRegion(self, frame, iamp):
        ...
        logging.debug(str('IlluRegion Amp ='+str(iamp)+' (x_start, x_end, y_start, y_end) = ' +
                          str(y_start) + ' ' + str(y_end)+' ' + str(x_start) + ' ' + str(x_end)))
    ...
    def OverscanRegion(self, frame, iamp):
        ...
        logging.debug(str('OverscanRegion Amp ='+str(iamp)+' (x_start, x_end, y_start, y_end) = '
                          + str(int(axis[0])-1) + ' ' + str(int(axis[1]))+' ' + str(int(axis[2])-1) + ' ' + str(int(axis[3]))))
